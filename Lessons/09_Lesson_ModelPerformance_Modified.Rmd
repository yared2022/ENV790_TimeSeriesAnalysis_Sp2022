...
observed_forecasted.df <- data.frame(date=electricity_price_processed$Month[(nobs-n_for+1):nobs], observed, MEAN_seas$mean, SNAIVE_seas$mean,SARIMA_for$mean)

ggplot(observed_forecasted.df, aes(x= date))+
  geom_line(aes(y=observed), color="black")+
  geom_line(aes(y=MEAN_seas$mean), color="green")+
  geom_line(aes(y=SNAIVE_seas$mean), color="blue")+
   geom_line(aes(y=SARIMA_for$mean), color="red")
