...
tsclean() from package 'forecast' identify and replace outliers and missing values in a time series using linear interpolation (local mean).
...
decomposed_data <- decompose(clean_wind_data)
plot(decomposed_data)


#Ex2: Store the three component in separate vector
  wind_random <- decomposed_data$random
  wind_trend <- decomposed_data$trend
  wind_season <- decomposed_data$seasonal

#Ex3: Create one data frame with all three vector and the Date columns from wind_data
df_wind_decomposed <- data.frame("date"= full_wind_data$Date, wind_trend, wind_random, wind_season)

#Ex4: Inspect the data frame using head() and tail()

head(df_wind_decomposed)
tail(df_wind_decomposed)
...
ggplot(decomposed_data) + geom_smooth(method = "lm")
