...
# Looking for alternative way to remove the first observation from the detrended by regression series directly 
#detrend_ts_us_TREP1 <- detrend_ts_us_TREP  [-1,]

combined_series_df <- data.frame(month=month(us_TREP1$my_date), us_TREP1, detrend_ts_us_TREP1, diff_us_TREP)
```
...
ggplot(combined_series_df, aes(x=month, y=TREP))
  geom_line()
...
acf(diff_us_TREP.df$TREP, ylim=c(-0.5,1), lag.max = 40,plot = TRUE)
```
### The more efficient method in eliminating the trend is detrending. Becuase detrending enables to estimate the trend and calculate the deviation from the estimated trend in a particular period. 
...
us_TREP_matrix <- matrix(us_TREP, byrow = FALSE, nrow = 12)
us_TREP_yearly <- colMeans(us_TREP_matrix)

my_year <- c(year(first(my_date)):year(last(my_date)))

us_TREP_yearly <- aggregate(us_TREP$my_date, by=list(month(month), year(my_date)), colMeans())
